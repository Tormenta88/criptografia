...
def are_images_same(main_image, sub_image, match_threshold=50):
    ...
    FLANN_INDEX_KDTREE = 1 #El que mejor va
    ...
    print(f"Cantidad de buenos matches: {len(goodMatches)}")

    return len(goodMatches) >= match_threshold, len(goodMatches)
...
for filename in os.listdir('sample_inputs/'):
    ...
    main_image = cv.imread('huellasBuscar/abuscar4.tif') # Poner a aquí la huella que queremos buscar
